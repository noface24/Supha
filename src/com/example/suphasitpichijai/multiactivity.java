...
public class multiactivity extends Activity {
	...
	private ThreadCountDownmulti threadCountDownmulti;

	private ThreadCheckEnd threadCheckEnd;
	public AQuery aq = new AQuery(this);
	private int count = 40;  // countdowntime
	public static int FinalScore=-1,score = 0;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		threadCountDownmulti = new ThreadCountDownmulti(count, this);

		threadCheckEnd = new ThreadCheckEnd(this);
		threadCountDownmulti.start();
		//threadCheckEnd.start();
		FinalScore=-1;
		score = 0;
	...
	public void threadProcess(int count) {
		...
		if (count <= 0) {
			//Toast.makeText(getApplicationContext(), "threadCountDownmulti.requestStop()", Toast.LENGTH_LONG).show();
			threadCountDownmulti.requestStop();
			FinalScore = score;
			threadCheckEnd.start();
	...
	public void threadProcesssEnd(int count2) {
		aq.ajax(connect.IPadress + "FinalScore", String.class,
				new AjaxCallback<String>() {

					private String overgame="";
					...
					public void callback(String url, String html,
						...
						if (!(html.trim().equals("-1"))) {
							threadCheckEnd.requestStop();
							if(score<Integer.parseInt(html.trim())){
								overgame = "แพ้";
							}else if(score>Integer.parseInt(html.trim())){
								overgame="ชนะ";
							}
							else{
								overgame="เสมอ";
							...
							// dialog
							AlertDialog.Builder alt_bld = new AlertDialog.Builder(multiactivity.this);
							alt_bld.setMessage(score+overgame)
									.setCancelable(false)
